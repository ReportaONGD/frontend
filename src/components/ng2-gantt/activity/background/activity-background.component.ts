...
export class GanttActivityBackgroundComponent implements OnInit {
    ...
    isDayWeekend(date: Date): boolean {
      if(this.zoomLevel !== Zooming[Zooming.month])
    ...
    private drawGrid(): void {
        if (this.zoomLevel === Zooming[Zooming.hours]) {
          ...
          this.timeScale.forEach((date: any) => {
              const index = this.cells.indexOf(date.getMonth() + 1 + '-' + date.getFullYear());
              if(index === -1)
                this.cells.push(date.getMonth() + 1  + '-' + date.getFullYear());
          });
