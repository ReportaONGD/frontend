...
@Component({
    ...
    template: `
        <div class="time_scale" [ngStyle]="setTimescaleStyle()">
            <div *ngIf="zoomLevel === 'day'" class="time_scale_line" [ngStyle]="setTimescaleLineStyle('none')">
          ...
          <div *ngIf="zoomLevel === 'month'" class="time_scale_line" [ngStyle]="setTimescaleLineStyle('1px solid #cecece')">
            <div class="time_scale_cell" *ngFor="let date of getMonths()" [ngStyle]="setTimescaleCellStyle()">{{date}}</div>
          </div>
        </div>`,
    styles: [`
        .weekend {
...
export class GanttTimeScaleComponent implements OnInit {
    ...
    private getMonths(): string[] {
      return this.ganttService.getMonths(this.timeScale);
    }
}
