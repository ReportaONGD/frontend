...
@Component({
    ...
    <div class="grid" #ganttGrid [ngStyle]="{ 'height': ganttActivityHeight, 'width':'50%'}">
    ...
    <div class="grid_data" #ganttGridData [ngStyle]="{ 'height': ganttService.calculateGanttHeight() }">
      <div #row *ngFor="let data of ganttService.groupData(ganttService.TASK_CACHE); let iGd = index" 
        ...
        <div class="grid_cell" [ngStyle]=" { 'width': gridColumns[0].width + 'px', 'padding-left': '20px'}"><label *ngIf="data.name === 'Planificada'"><a (click)="edit($event, data)" href="#">{{data.treePath}}</a></label></div>
        ...
        <div class="grid_cell" [ngStyle]=" { 'width': gridColumns[1].width + 'px', 'padding-left': ganttService.isChild(data.treePath) }"> 
          ...
          <!--<div class="grid_cell" [ngStyle]="{ 'width': gridColumns[2].width + 'px' }">
              &lt;!&ndash;<div>{{ ganttService.isParent(data.treePath) === true ? ganttService.calculateParentTotalPercentage(data, project.tasks) : data.percentComplete }}</div>&ndash;&gt;
    ...
    </div><div class="gantt_activity" (window:resize)="onResize($event)" [ngStyle]="{ 'height': ganttActivityHeight, 'width': '50%'}">
...
export class GanttActivityComponent implements OnInit, DoCheck {
    ...
    @Output() editCronograma: EventEmitter<any> = new EventEmitter<any>();
    ...
    public gridColumns: any[] = [
        { name: 'Objetivo-Resultado-Actividad', left: 20, width: 300 },
        { name: 'Tipo', left: 20, width: 100 },
        { name: 'Duración', left: 14, width: 140 }
    ];
    ...
    ngOnInit() {
        ...
        this.ganttService.TIME_SCALE = this.ganttService.calculateScale(this.options.scale.start, this.options.scale.end, this.options.zooming);
    ...
    edit(event:Event, data) {
      event.preventDefault();
      event.stopPropagation();
      this.editCronograma.emit(data);
    }
