...
export class GanttService {
    ...
    private barStyles: IBarStyle[] = [
        ...
        { status: "progress", backgroundColor: "#EF5350", border: "1px solid #C62828", progressBackgroundColor: "#C62828" }
    ];
    ...
    private calculateBarWidth(start: Date, end: Date, hours?: boolean, months?: boolean): number {
        ...
        let monthsDif =this.calculateDiffMonths(start, end);
    ...
    private calculateBarLeft(start: Date, scale: any[], hours?: boolean, months?: boolean): number {
        var left: number = 0;
        ...
        if (start != null) {
            ...
            if(!months)
            {
              ...
              const m = this.getMonths(scale);
              let d = (start.getMonth() + 1).toString();
              if(d.length === 1)
                d = '0' + d;
              let elementTask = m.filter(p => d + '-' + start.getFullYear() === p);
              let position = m.indexOf(elementTask[0]);
              left = position * this.cellWidth + position;
            }
        }
        return left;
    }
    ...
    private calculateBarLeftDelta(start: Date, hours?: boolean, months?: boolean): number {
        var offset: number = 0;
        var hoursInDay: number = 24;
    ...
    public calculateBar(task: any, index: number, scale: any, hours?: boolean, months?: boolean) {
        var barStyle = this.getBarStyle(task.status);
        //console.log(this.barTop * index + 2)
        return {
            'top': this.barTop * index + 2 + 'px',
            'left': this.calculateBarLeft(task.start, scale, hours, months) + 'px',
            'height': this.barHeight + 'px',
            'line-height': this.barLineHeight + 'px',
            'width': this.calculateBarWidth(task.start, task.end, hours, months) + 'px',
    ...
    public calculateDiffMonths(start: Date, end: Date): number {
      ...
      try {
        if(yearStart === yearEnd) {
          ...
          const diffYears = yearEnd - yearStart;
          if (diffYears === 1) {
            months = (12 - monthStart) + monthEnd;
          }
          else {
            months = (12 - monthStart) + (12 * diffYears) + monthEnd;
          }
        }
        return months;
      } catch (err) {
    ...
    public calculateScale(start: Date = new Date(), end: Date = this.addDays(start, 7), zoom? : any) {
        let scale: any[] = [];
    ...
    public getMonths(timeScale: any): string[] {
      const months: string[] = [];
      timeScale.forEach((date: any) => {
        let m = (date.getMonth() + 1).toString();
        if (m.length === 1) {
          m = '0' + m;
        }
        const index = months.indexOf(m + '-' + date.getFullYear());
        if (index === -1) {
    ...
    public doTaskCheck(tasks: any[], treeExpanded: boolean): boolean {
        // borrando la siguiente linea habilita cache
