...
@Injectable()
export class GastosDocumentosService extends BaseService {
  constructor( api: ApiService, private http: HttpClient) {
    super(`${environment.apiEndpoint}${EndPoints.gasto_documento}`, api);
  }
  ...
  download(item: DocumentoModel): Observable<any> {
    const ruta = `${environment.apiEndpoint}${EndPoints.gasto_documento_download}`;
