...
@Injectable()
export class ObjetivoEspecificoFuenteVerificacionDocumentoService extends BaseService {
  constructor( api: ApiService, private http: HttpClient) {
    super(`${environment.apiEndpoint}${EndPoints.oe_fuente_verificacion_documento}`, api);
  }
  ...
  download(item: DocumentoModel): Observable<any> {
    const ruta = `${environment.apiEndpoint}${EndPoints.oe_fuente_verificacion_documento_download}`;
